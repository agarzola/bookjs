(function () {
    ...
    for (var i = 0, img, figure; i < imagePaths.length; i++) {
      ...
      if (i === 0) {
        img.addEventListener('load', function (event) {
          var ratio = getRatio(Book, this);
          Book.styles = setStyles(Book, ratio);
        });
      }
  ...
  function setStyles(Book, ratio) {
    styles = styles.replace(/\$ratio/, (ratio + %));
    styles = styles.replace(/\$page_turn_duration/,
      (Book.page_turn_duration + 's'));
    styles = styles.replace(/\$page_turn_duration_half/,
      (Book.page_turn_duration_half + 's'));
    return styles;
  }
})();
